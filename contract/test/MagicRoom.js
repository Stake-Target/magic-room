...
contract("MagicRoom", (accounts) => {
  ...
  context("create a new room", async () => {
    it("should be able to create a new room", async () => {
      const result = await createRoom('alice', alice);
    ...
    it("should not allow two room", async () => {
      ...
      await createRoom('alice', alice);
      await utils.shouldThrow(contractInstance.createRoom('bob', toWei(2), { from: bob }));
    })
  })
  ...
  context("view fns", async () => {
    ...
    it("should return current room", async () => {
      await giveTokens(bob, 100);
      await contractInstance.createRoom('bob', toWei(1), { from: bob });
      const result = await contractInstance.getCurrentRoom.call();
  ...
  context("enter to room", async () => {
    it("on chair", async () => {
      await createRoom('alice', alice);
      await giveTokens(bob, 100);
      await contractInstance.enterToRoom('bob', toWei(2), { from: bob });
    ...
    it("small amount", async () => {
      await createRoom('alice', alice);
      await giveTokens(bob, 100);
      await utils.shouldThrow(contractInstance.enterToRoom('bob', toWei("0.0001"), { from: bob }));
    })
    it("amount should go up", async () => {
      ...
      const resultBob = await contractInstance.enterToRoom('bob', toWei(2), { from: bob });
      const resultCavin = await contractInstance.enterToRoom('cavin', toWei(3), { from: cavin });

      expect(resultBob.receipt.status).to.equal(true);
      expect(resultCavin.receipt.status).to.equal(true)
      await utils.shouldThrow(contractInstance.enterToRoom('cavin', toWei(3), { from: cavin }));
    })
    it("rewards", async () => {
      await createRoom('bob', bob);
      await giveTokens(bob, 99);
      await giveTokens(cavin, 100);
      await contractInstance.enterToRoom('cavin', toWei(2), { from: cavin });
  ...
  context("room", async () => {
    it("should change lastActionTime", async () => {
      await createRoom('alice', alice, 1);
      ...
      for(let i = 1, prevPrice = fromWei(room.price) + 1; i < +room.steps; i++, prevPrice ++) {
        await contractInstance.enterToRoom('bob', toWei(prevPrice), { from: bob });
    ...
    it("should finish", async () => {
      ...
      for(let i = 1; i < +room.steps; i += 3) {
        await nextBid('bob', bob);
        await nextBid('cavin', cavin);
        await nextBid('dany', dany);
      }
      async function nextBid (name, address) {
        prevPrice += 1;
        step += 1;
        await contractInstance.enterToRoom(name, toWei(prevPrice), { from: address });
      }
      const room2 = await contractInstance.getCurrentRoom.call();
      expect(room2.active).to.equal(false);
    })
    it("name length should less 20", async () => {
      await utils.shouldThrow(createRoom('aliceeeeeeeeeeeeeeeee', alice, 1));
      await utils.shouldThrow(createRoom('aliceeeeeeeeeeeeeee🔈', alice, 1));
    })
    // it("timeout", async () => {
    //   await createRoom('alice', alice, 1);
    //   await giveTokens(bob, 10000);
    ...
    it("rewards", async () => {
      const tokenimic = await contractInstance.getTokenimic.call();
      const cavinTikens = 2;
      await createRoom('bob', bob, 1);
      await giveTokens(cavin, cavinTikens);
      const reward = cavinTikens * tokenimic.reward / 100;
      await contractInstance.enterToRoom('cavin', toWei(cavinTikens), { from: cavin });
  ...
  async function createRoom (name, from, amount = 1) {
    ...
    return contractInstance.createRoom(name, toWei(amount), { from });
  }
})
